...
Target R2 structure (per R2_STORAGE_STRUCTURE.md):
    {storage_prefix}/native-builds/{addon_version}/{module}/{platform}/{arch}/{artifact}
...
def _infer_manifest_versions(manifest_dir: Path) -> list[str]:
    versions = {
        rel.parts[0]
        for rel in (path.relative_to(manifest_dir) for path in manifest_dir.rglob("*.json"))
...
def main() -> int:
    ...
    parser.add_argument(
        "--addon-version",
        default="",
        help="Addon semantic version. If omitted, inferred from --manifest-dir when possible.",
    )
    ...
    addon_version = args.addon_version.strip()
    if not addon_version and manifest_dir.is_dir():
        inferred_versions = _infer_manifest_versions(manifest_dir)
        if len(inferred_versions) == 1:
            addon_version = inferred_versions[0]
        elif len(inferred_versions) > 1:
            raise RuntimeError(
                "Multiple addon versions found in manifest-dir; pass --addon-version explicitly: "
                + ", ".join(inferred_versions)
            )
    if not addon_version:
        raise RuntimeError("addon version is required (pass --addon-version or provide manifest-dir)")
    ...
    for spec in RUNTIME_ARTIFACT_SPECS:
        ...
        for module_name in MODULE_NAMES:
            ...
            key = (
                f"{args.storage_prefix}/native-builds/{addon_version}/{module_name}/"
    ...
    latest_payload = {
        "product_id": args.storage_prefix,
        "addon_version": addon_version,
        "updated_at": int(time.time()),
    }
    latest_key = f"{args.storage_prefix}/native-builds/latest.json"
    latest_path = Path("dist") / ".native-builds-latest.json"
    latest_path.parent.mkdir(parents=True, exist_ok=True)
    latest_path.write_text(json.dumps(latest_payload, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    print(f"[r2-upload] {latest_path.name} -> s3://{args.bucket}/{latest_key}")
    _upload_file(s3, latest_path, args.bucket, latest_key)
    uploaded += 1
