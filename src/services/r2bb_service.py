...
class R2BBBackendService(NativeLicensedFeatureService):
    ...
    def normalize_mapping_entries(self, entries):
        ...
        return self.get_backend().normalize_mapping_entries(entries)

    def mapping_entries_to_pairs(self, entries):
        self.require_feature_unlocked()
        return self.get_backend().mapping_entries_to_pairs(entries)

    def mapping_entries_to_export_bones(self, entries):
        self.require_feature_unlocked()
        return self.get_backend().mapping_entries_to_export_bones(entries)

    def mapping_entries_to_export_name_map(self, entries):
        self.require_feature_unlocked()
        return self.get_backend().mapping_entries_to_export_name_map(entries)

    def mapping_entries_to_rotation_axis_signs(self, entries):
        self.require_feature_unlocked()
        return self.get_backend().mapping_entries_to_rotation_axis_signs(entries)

    def mapping_entries_to_transform_axis_signs(self, entries):
        self.require_feature_unlocked()
        return self.get_backend().mapping_entries_to_transform_axis_signs(entries)
